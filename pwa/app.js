...
// Removed idToken, userEmail, isAuthenticated flags
...
const revisitNeededSelect = document.getElementById('revisit-needed');
 ...
 // --- Event Listeners ---
 document.getElementById('login-btn').onclick = handleLogin; // Re-added listener for original button
...
function handleLogin() {
    // Reverted to original Implicit Grant flow
...
function handleRedirectHash() {
    // Reverted to original hash handling
    ...
    if (hash) {
        ...
        if (token) {
            accessToken = token; // Store access token
            localStorage.setItem('access_token', accessToken);
            // Reload was causing issues, let onload handle UI update now
...
// --- API Calls (Modified to send Access Token) ---

// Check-in Call
async function handleSelectPlaceFromMap(pId, pName, pAddress) {
  if (!accessToken) { alert('請先登入'); handleLogin(); return; } // Check accessToken
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/checkin`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + accessToken }, // Send Access Token
      mode: 'cors',
...
// Removed global function declarations for GIS
