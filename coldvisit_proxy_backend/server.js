...
async function verifyGoogleOAuthToken(accessToken) {
  ...
  try {
    ...
    // Create an OAuth2 client using the user's access token
    // Note: We don't need our backend client ID here, just the token
    ...
    // Handle potential errors like invalid token (401/403) or API not enabled
...
const authenticateUser = async (req, res, next) => {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
        ...
        userEmail = await verifyGoogleOAuthToken(token); // Use new verification function
...
app.post('/api/checkout', authenticateUser, async (req, res) => {
  ...
  try {
    ...
    const durationSec = checkInTime ? Math.round((now - checkInTime) / 1000) : 0; // Changed calculation

    // 4. Update Sheet (Columns G to V, H is now seconds)
    const valuesToUpdate = [
        { range: `${SHEET_NAME}!G${rowIndex}`, values: [[now.toISOString()]] }, { range: `${SHEET_NAME}!H${rowIndex}`, values: [[durationSec]] }, // Use durationSec
    ...
    // Update response key
    res.json({ success: true, checkOutTime: now.toISOString(), durationSeconds: durationSec }); // Changed key
